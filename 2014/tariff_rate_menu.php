...
if(isset($_POST['tariff_reg']))
{
	...
	$query = "insert into tariff_rate(service_service_id,carname_master_id,rate,minimum_chg_km,extra_km_rate,minimum_chg_hourly,extra_hour_rate)
	values('".$_POST['service_service_id']."','".$_POST['carname_master_id']."','".$_POST['rate']."'
	, '".$_POST['minimum_chg_km']."','".$_POST['extra_km_rate']."','".$_POST['minimum_chg_hourly']."','".$_POST['extra_hour_rate']."'
	)";	
	$cust_result = $data_base_connect_object->execute_query_return("select id from customer_reg");
	...
	values('".$row['id']."','".$_POST['carname_master_id']."','".$_POST['service_service_id']."','".$_POST['rate']."',
	'".$_POST['minimum_chg_km']."','".$_POST['extra_km_rate']."','".$_POST['minimum_chg_hourly']."','".$_POST['extra_hour_rate']."'
	)";
		$data_base_connect_object->execute_query_update($cust_query,"multi_entry");
	}
	$supplier_result = $data_base_connect_object->execute_query_return("select `supplier_id` from `supplier_reg`");
	while($row=mysql_fetch_array($supplier_result))
	{
		$supp_query="insert into `supplier_tariff`(`supplier_reg_name`,`carname_master_id`,`service_service_id`,`rate`,`minimum_chg_km`,`extra_km_rate`,`minimum_chg_hourly`,`extra_hour_rate`)
	values('".$row['supplier_id']."','".$_POST['carname_master_id']."','".$_POST['service_service_id']."','".$_POST['rate']."',
	'".$_POST['minimum_chg_km']."','".$_POST['extra_km_rate']."','".$_POST['minimum_chg_hourly']."','".$_POST['extra_hour_rate']."'
	)";
		$data_base_connect_object->execute_query_update($supp_query,"multi_entry");
	}
	$data_base_connect_object->execute_query_update($query,"tariff_reg");
}
if(isset($_GET['reg']))
{
	echo "<script language=\"javascript\">
		alert('New Tariff Rate Added SuccessFully .');
		window.location='tariff_rate_menu.php';
	</script>
	";
}
if(isset($_GET['dell']))
...
<?php
	$data_base_connect_object =new DataBaseConnect(); 
	$my_query="select * from sub_module_assign where `login_id` = '".$login_id."' && `submodule_id` = '8' ";
	...
	{    
	?>                 
<a href="tariff_rate_menu.php" class="btn red"><i class="icon-ok"></i> Add</a>
<?php
	}
	if($edit_status==1)
	{
...
<?php
	...
	{
		?>
<a href="tariff_rate_menu_serch.php" class="btn blue"><i class="icon-search"></i> Search</a>
<?php
	}
?>
</div> 
...
<?php
	if($add_status==1)
	{
		?>
                    ...
                    <h4><i class=" icon-cloud"></i>Tariff Rates</h4>
                    </div>
				...
				<tr><td></td><td>&nbsp;</td></tr>
                  ...
                  <button type="submit" value="Add" style="margin-left:30%" class="btn green" name="tariff_reg"/><i class="icon-ok"></i> Submit</button>
                     </div>
                  </div>
                     </div> 
                      <?php
