 ...
 if(isset($_POST['supplier_tariff_reg']))
{
	...
	$query = "insert into `supplier_tariff`(`supplier_reg_name`,`carname_master_id`,`service_service_id`,`rate`,`minimum_chg_km`,`extra_km_rate`,`minimum_chg_hourly`,`extra_hour_rate`)
	values('".$_POST['supplier_reg_name']."','".$_POST['carname_master_id']."','".$_POST['service_service_id']."','".$_POST['rate']."',
	'".$_POST['minimum_chg_km']."','".$_POST['extra_km_rate']."','".$_POST['minimum_chg_hourly']."','".$_POST['extra_hour_rate']."'
	)";	
	$data_base_connect_object->execute_query_update($query,"supplier_tariff_reg");
}
if(isset($_GET['reg']))
{
	echo "<script language=\"javascript\">
		alert('Customer Rate Added SuccessFully .');
		window.location='supplier_tariff_rate_menu.php';
	</script>
	";
}
if(isset($_GET['dell']))
...
<?php
	$data_base_connect_object =new DataBaseConnect(); 
	$my_query="select * from sub_module_assign where `login_id` = '".$login_id."' && `submodule_id` = '11' ";
	...
	{    
	?>   
<a href="supplier_tariff_rate_menu.php" class="btn red"><i class="icon-ok"></i> Add</a>    
<?php
	}
	if($edit_status==1)
	{
		?>                    
<a href="supplier_tariff_rate_menu_edit.php" class="btn blue"><i class="icon-edit"></i> Edit</a>
<?php
	}
	if($delete_status==1)
	{
		...
		?>
              ...
              <select name="service_service_id" class="chosen" >	
				...
				<tr><td></td><td>&nbsp;</td></tr>
                  ...
                  <button type="submit" value="Add" style="margin-left:30%" class="btn green" name="supplier_tariff_reg"/><i class="icon-ok"></i> Submit</button>
                     </div>
