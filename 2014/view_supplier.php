...
<a href="supplier_menu_serch.php" class="btn red"><i class="icon-search"></i> Search</a>
</div> 
<br />
...
if(mysql_num_rows($result)>0)
{
	...
	$supply_all=mysql_query("select * from `supplier_master` where `id` = '$supplier_master_id'  ");
	$ftc_supply=mysql_fetch_array($supply_all);
	$type = $ftc_supply['type'];
	$supplier_master_name_id=$row['supplier_master_name_id'];
	$supply_master=mysql_query("select * from `supplier_master_type` where `id` = '$supplier_master_name_id'  ");
	$ftc_supply_master=mysql_fetch_array($supply_master);
	$name = $ftc_supply_master['name'];
...
<table width="100%" align="center" class="table table-bordered table-hover" style="text-align:center;">
  ...
  <tr>
    ...
    <td width="13%"><?php echo $type;?></td>
    <td width="13%"><strong>Supplier Category:</strong></td>
    <td width="13%"><?php echo $name;?></td>
