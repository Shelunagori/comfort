...
if(isset($_POST['duty_slip']))
{
	...
	if(empty($_POST['car_reg_name']))
	{
		...
		$car_id=$row_carid['car_id'];
		$old_car_no = $car_id;
	}
	else
	{
		$old_car_no = $_POST['car_reg_name'];
	}
	if(!empty($_POST['closing_km']))
	{
	...
	$days=(($main2-$main1)/86400);
		...
		$res = $data_base_connect_object->execute_query_return("select night_hault_charges from carname_master where id='".$carname_master_id."'");
		$row= mysql_fetch_array($res);
		$night_hault_charges = $days * $row['night_hault_charges'];		
		$total_km = $_POST['closing_km']-$opening_km;
	...
	/*	if($days>0)
		...
		*/
		...
		$query = "INSERT INTO `duty_slip` (`current_date`, `guest_name`,`contactnumber`,`photo_id`, `service_service_id`, `carname_master_id`,`car_reg_name`,`new_car_no`, `customer_reg_name`, 
	`detail_number`, `driver_reg_driver_id`, `opening_km`, `opening_time`, `closing_km`, `closing_time`, `date_from`, `date_to`,
	 `extra_chg`, `permit_chg`, `parking_chg`, `otherstate_chg`, `guide_chg`, `misc_chg`, `remarks`,
	 `authorized_person`,`date_authorization`,`reason`,
	 `status`,`total_km`,`rate`,`amount`,`loginname`,`countername`) VALUES
	('".date('Y-m-d')."', '".$_POST['billing_name']."','".$_POST['contactnumber']."',
	'".$_POST['photo_id']."' , '".$_POST['service_service_id']."'
 	,'".$_POST['carname_master_id']."', '".$old_car_no."', '".$_POST['new_car_no']."',
	'".$_POST['customer_reg_name']."', '".$_POST['detail_number']."', '".$_POST['driver_reg_driver_id']."', '".$_POST['opening_km']."', '".$opening_time."', '".$_POST['closing_km']."', '".$closing_time."', '".DateExact($_POST['date_from'])."', '".DateExact($_POST['date_to'])."','".$_POST['extra_chg']."','".$_POST['permit_chg']."','".$_POST['parking_chg']."','".$_POST['otherstate_chg']."','".$_POST['guide_chg']."','".$_POST['misc_chg']."','".$_POST['remarks']."'
	,'".$_POST['authorized_person']."', '".DateExact($_POST['date_authorization'])."' ,'".$_POST['reason']."'
	,'no','".$total_km."','".$_POST['rate']."','".$amount."','".$_SESSION['username']."','".$_SESSION['counter']."')";	
	
	}
	
	else {
		$query = "INSERT INTO `duty_slip` (`current_date`, `guest_name`,`photo_id`, `service_service_id`, `carname_master_id`, `car_reg_name`, `new_car_no`, `customer_reg_name`, 
		`detail_number`, `driver_reg_driver_id`, `opening_km`, `opening_time`, `closing_km`, `closing_time`, `date_from`, `date_to`,
		 `extra_chg`, `permit_chg`, `parking_chg`, `otherstate_chg`, `guide_chg`, `misc_chg`, `remarks`
		 ,`authorized_person`,`date_authorization`,`reason`,
		 `status`,`total_km`,`rate`,`amount`,`loginname`,`countername`) VALUES
		('".date('Y-m-d')."', '".$_POST['billing_name']."',
		'".$_POST['photo_id']."' , '".$_POST['service_service_id']."'
		,'".$_POST['carname_master_id']."', '".$old_car_no."',  '".$_POST['new_car_no']."',
		'".$_POST['customer_reg_name']."', '".$_POST['detail_number']."', '".$_POST['driver_reg_driver_id']."', '".$_POST['opening_km']."', '".$opening_time."', '0', '".$closing_time."', '".DateExact($_POST['date_from'])."', '0000-00-00','".$_POST['extra_chg']."','".$_POST['permit_chg']."','".$_POST['parking_chg']."','".$_POST['otherstate_chg']."','".$_POST['guide_chg']."','".$_POST['misc_chg']."', '".$_POST['remarks']."'
		,'".$_POST['authorized_person']."', '".DateExact($_POST['date_authorization'])."' ,'".$_POST['reason']."'
		,'no','0','".$_POST['rate']."','0','".$_SESSION['username']."','".$_SESSION['counter']."')";
	
	}
	
	$data_base_connect_object->execute_query_update($query, "duty_slip");
	$result =$data_base_connect_object->execute_query_return("select max(dutyslip_id) from duty_slip");
	$row= mysql_fetch_array($result);
	$max_dutyslip_id = $row[0];
				
$result=$data_base_connect_object->execute_query_return("select * from `duty_slip` where `dutyslip_id`='".$max_dutyslip_id."'");
...
$time_duration=$hours.'.'.$minutes;
		...
		if($opening_date_new=="31-11--0001")
		{
			$opening_date_new="";
		}
...
<body class="fixed-top">
   ...
   <div class="page-container row-fluid">
      ...
      <div class="page-content">
         <div class="container-fluid"> <a class="btn green diplaynone"   style="line-height:30px" role="button"  href="javascript:window.print();" ><i class="icon-print"></i>Print</a>
     <?php menu(); ?>
               <?php 
			   for($i=1;$i<=2;$i++)
			   {
...
<table  width="100%" align="center" style="line-height:25px" border="1" class="dataTable<?php if($i % 2==0) { ?>  prnt<?php } ?>" >
<thead>
  <tr>
