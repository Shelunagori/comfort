...
$result_ledger=mysql_query("select `bill_number`,`name`,`date` from `ledger` where `type`='' && `ledger_type`='Customer' && `bill_number`>='2705'");
...
//@mysql_query("insert into `ledger` set `ledger_type`='Ledger' , `credit`='".$car_higher_service_amnt."' ,  `name`='Car Hire Services' , `bill_number`='".$invoice_id."', `narration`='".$remarks."'");			
	...
	echo "Invoice No. ".$invoice_id." Date ".date("d-m-Y",strtotime($row_ledger['date']))." Car Hire Service Amount ".$car_higher_service_amnt;
